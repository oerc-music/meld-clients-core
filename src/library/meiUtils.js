...
function countMeasures(MEIObject) {
  ...
  var measureCount = MEIObject.evaluate(
    "count(//mei:measure)",
    MEIObject,
    nsResolver,
...
function findMeasures(n, MEIObject) {
  ...
  var staves = MEIObject.evaluate(
    "//mei:staff[@n=" + n + " and .//mei:note]",
    MEIObject,
    nsResolver,
    XPathResult.ORDERED_NODE_ITERATORTYPE,
    null,
  );
...
function InstrumentMeasure(barStaff, MEIObject) {
  ...
  this.barNo = this.MEIObject.evaluate(
    "./ancestor::mei:measure/@n",
    barStaff,
    nsResolver,
    XPathResult.NUMBER_TYPE,
    null,
  ...
  this.events = [];
  this.duration = 0;
  var eventObjs = this.MEIObject.evaluate(
    "./mei:layer//mei:note | ./mei:layer//mei:rest | ./mei:layer//mei:chord",
    barStaff,
    nsResolver,
    XPathResult.ORDERED_NODE_ITERATORTYPE,
    null,
  ...
  while (event) {
    ...
    if (
      this.events.length &&
      this.events[this.events.length - 1].extends(event)
    ) {
...
function findInstruments(MEIObject, instrumentSet) {
  ...
  var staffDefs = MEIObject.evaluate(
    "//mei:staffDef",
    MEIObject,
    nsResolver,
    XPathResult.ORDERED_NODE_ITERATORTYPE,
  ...
  while (staffDef) {
    ...
    instruments[staffDef.getAttributeNS(null, "n") - 1] = new Instrument(
      staffDef,
      MEIObject,
      instrumentSet,
    );
...
var Instruments = {
  ...
  "cor anglais": new InstrumentType(
    false,
    "Cor anglais",
    "ca",
    "Woodwind",
  ...
  "english horn": new InstrumentType(
    false,
    "Cor anglais",
    "ca",
    "Woodwind",
  ...
  "bass clarinet": new InstrumentType(
    false,
    "Bass Clarinet",
    "Bscl.",
    "Woodwind",
  ...
  "a bass clarinet": new InstrumentType(
    false,
    "A Bass Clarinet",
    "Bscl.A",
    "Woodwind",
  ...
  "f trumpets (1-3)": new InstrumentType(
    false,
    "F Trumpets (1-3)",
    "trps.F",
    "Brass",
  ...
  "trombones (1-3)": new InstrumentType(
    false,
    "Trombones (1-3)",
    "trbs.F",
    "Brass",
  ...
  "bass tuba in e♭": new InstrumentType(
    false,
    "E♭ Bass tuba",
    "Bstb",
    "Brass",
  ...
  "violoncello": new InstrumentType(
    false,
    "Cello",
    "vc",
    "Strings",
  ...
  "women and noble boys soprano": new InstrumentType(
    false,
    "Women & boys S",
    "S",
    "Cast",
  ...
  "women and noble boys alto": new InstrumentType(
    false,
    "Women & boys A",
    "A",
    "Cast",
...
export function Orchestration(MEIString, additionalInstruments) {
  ...
  this.instrumentSet = { ...Instruments, ...additionalInstruments };
  var p = new DOMParser();
...
function Instrument(staffDef, MEIObject, instrumentSet) {
  ...
  this.caption = function (SVG, x, y, active) {
    if (!this.type)
      this.type = getInstrumentType(this.name, this.instrumentSet);
    if (this.type) {
...
function singleInstrument(set) {
  return set.every((x) => x.type && x.type.name === set[0].type.name)
    ? set[0].type.name
    : false;
}
function singleSection(set) {
  return set.every((x) => x.type && x.type.section === set[0].type.section)
    ? set[0].type.section
    : false;
}
export function caption(
  ...
  if (active) {
    if (set.length === 1) {
      ...
      return {
        obj: (
          <g onMouseOver={mover} onMouseOut={mout}>
            <text x={x} y={y} className={baseclass + cl}>
              {set[0].name}
            </text>
          </g>
        ),
      ...
      section &&
      orchestra.filter((x) => x.type.section === section).length === set.length
    ) {
    ...
    if (set.length === 1) {
      ...
      inst &&
      orchestra.filter((x) => x.type.name === inst).length === set.length
    ) {
      cl = section + " " + inst + " nnn" + n;
      return {
        obj: (
...
function isAllOfInstrument(set, orchestra) {
  ...
  if (set.length) {
    ...
    if (set.every((x) => x.type.name === instr)) {
      return (
        orchestra.filter((x) => x.type.name === instr).length === set.length
      );
    }
  }
  return false;
...
function entireSection(set, orchestra) {
  ...
  if (set.length) {
    ...
    if (set.every((x) => x.type.section === section)) {
      return (
        orchestra.filter((x) => x.type.section === section).length ===
        set.length
      );
    }
  }
