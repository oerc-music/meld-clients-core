...
function getContext(node) {
  ...
  return (
    <div className="motifList" onClick={(x) => node.scrollIntoView()}>
      ...
      <span className="preContent">{textBits[0]}</span>
      <span className="content">{textBits[1]}</span>
      <span className="postContent">{textBits[2]}</span>...
    </div>
  );
}
export default class EssayLinks extends Component {
  ...
  allRefsIndex() {
    ...
    for (var i = 0; i < this.props.iterations.length; i++) {
      ...
      if (linkDivs.length) {
        blocks.push(
          <div className="motifBlock">
            <div className="motifName">
              {this.props.iterations[i][prefix.rdfs + "label"]}
            </div>
            {linkDivs}
          </div>,
        );
  ...
  linkForIteration(uri) {
    ...
    var audioURI = iteration.embodimentLists.MP3
      ? iteration.embodimentLists.MP3[0]
      : false;
    var audio = iteration.embodimentLists.MP3 ? (
      <Media key={iteration.embodimentLists.MP3[0]}>
        <div className="media">
    ...
    return (
      <div className="linkBlock" key={"itLink" + uri}>
  ...
  render() {
    ...
    return (
      <Tabs defaultIndex={0} className="linksTab">
        ...
        <TabPanel>
          <div className="links">{visibles}</div>
        </TabPanel>
        <TabPanel>
          {" "}
          <div className="motifListBox">{linkDivs}</div>
        </TabPanel>
      </Tabs>
    );
  }
