...
class AnnotationsListing extends Component {
  ...
  render() {
    ...
    if (
      Object.keys(this.props.score).length &&
      Object.keys(this.props.score.componentTargets).length
    ) {
                ...
                <div className="annotationTarget">
                  {annotation["oa:hasTarget"].map((t) => {
                    if (t["@id"] in this.props.score.componentTargets) {
                      return (
                        <span
                          className={annotation["oa:motivatedBy"][
                            "@id"
                          ].replace(":", "_")}
                          title={t["@id"]}
                          key={t["@id"]}
                          onClick={() => {
                            this.props.scorePageToComponentTarget(
                              ...
                              this.props.score.componentTargets[t["@id"]][
                                "MEI"
                              ][0],
                              this.props.scoreUri,
                      ...
                      return <span>Loading component target: {t["@id"]}</span>;
                    }
                  })}
