...
class TEI extends Component {
  ...
  scrollToURI() {
    if (this.props.scrollToURI && this.containerDiv) {
      var fragment = this.props.scrollToURI.substring(
        this.props.scrollToURI.indexOf("#") + 1,
      );
      var el = this.containerDiv.getElementByID(fragment);
      if (el) {
        textBox.scrollTop =
          el.offsetTop - textBox.offsetTop + textBox.clientHeight / 2;
      }
    }
  }
  resize() {
  ...
  render() {
    ...
    if (
      Object.keys(this.props.tei.TEI).length &&
      this.props.uri in this.props.tei.TEI
    ) {
      // HACK //
      if (this.props.uri.indexOf("commentaries") > -1) {
        if (
          this.props.onMotifChange &&
          (!this.props.motif || !this.props.uri.endsWith(this.props.motif))
        ) {
          return <div />;
        }
        return (
          <div
            ...
            id={this.props.uri.substr(
              this.props.uri.indexOf("commentaries/") + 13,
            )}
          />
        );
      } else {
  ...
  returnHTMLizedTEI() {
    ...
    if (this.props.title) {
      TEIhtml = TEIhtml.replace(
        /<tei-title data-teiname="title">.*tei-title>/gi,
  ...
  componentDidUpdate() {
    ...
    this.props.annotations.map((annotation) => {
      ...
      if (!Array.isArray(annotationTargets))
        annotationTargets = [annotationTargets];
      // each annotation...
      var frags = annotationTargets.map((annotationTarget) => {
        // each annotation target
        if (
          (annotationTarget["@id"].startsWith(this.props.uri) &&
      ...
      if (!Array.isArray(annotationBodies))
        annotationBodies = [annotationBodies]; // each annotation...
      frags = annotationBodies.map((annotationBody) => {
        // each annotation target
        if (
          (annotationBody["@id"].startsWith(this.props.uri) &&
            annotationBody["@id"].indexOf("#") > -1) ||
          annotationBody["@id"][0] == "#"
        ) {
      ...
      if (
        pref.oa + "motivatedBy" in annotation &&
        annotation[pref.oa + "motivatedBy"]["@id"] ===
          pref.meldterm + "personInfo"
      ) {
  ...
  findOtherRefs() {
    ...
    for (var i = 0; i < refs.length; i++) {
      ...
      var perf = this.props.performances.find(
        (x) => x[pref.frbr + "partOf"]["@id"] == refURI,
      );
      if (perf) {
  ...
  cancelHighlights() {
    ...
    for (var i = rules.length - 1; i >= 0; i--) {
      if (
        ...
        rules[i].selectorText.indexOf("temporary") > -1
      ) {
  ...
  addHighlight(highlight) {
    ...
    var style = this.props.highlightStyle
      ? this.props.highlightStyle
      : { fill: "blue", stroke: "blue !important", color: "blue" };
    ...
    if (pref.oa + "motivatedBy" in annotation) {
      switch (idOrFirstId(annotation[pref.oa + "motivatedBy"])) {
        case pref.meldterm + "updateViewerState":
          ...
          for (var i = 0; i < annotation[pref.oa + "hasBody"].length; i++) {
            ...
            if (thing[pref.meldterm + "highlight"])
              highlight = highlight.concat(thing[pref.meldterm + "highlight"]);
          }
          for (var i = 0; i < fragments.length; i++) {
            var fragFullString = fragments[i]["@id"];
            var fragString = fragFullString.substring(
              fragFullString.indexOf("#") + 1,
            );
        ...
        case pref.meldterm + "definition":
          var definition = {
            definition: objOrFirstObj(annotation[pref.oa + "hasBody"])[
              pref.meldterm + "hasDefinition"
            ],
          };
          for (var i = 0; i < fragments.length; i++) {
      ...
      annotation["oa:hasBody"].map((b) => {
        ...
        if (b["@id"] === MARKUP_EMPHASIS) {
          ...
          this.props.handleCueImage(
            ReactDOM.findDOMNode(this),
            annotation,
            this.props.uri,
            fragments,
            this.props.tei.fragImages,
          );
        } else {
...
function setCSSProp(selector, obj) {
  ...
  while (
    i < rules.length &&
    (!rules[i].selectorText || rules[i].selectorText.indexOf(selector) === -1)
  ) {
    i++;
  }
  if (i == rules.length) {
