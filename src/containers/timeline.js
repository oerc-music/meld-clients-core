    ...
    for (var i = 0; i < structures.length; i++) {
      if (Array.isArray(structures[i][2])) {
        ...
        if (Number.isInteger(structures[i][2])) {
          ...
          console.log(
            "mangled dramatic structure: ",
            structures[i],
            Array.isArray(structures[i][2]),
          );
        }
      }
    }
  ...
  render() {
    if (this.state.structures) {
      ...
      for (var i = 0; i < structures.length; i++) {
        ...
        divLines.push(
          <text className="actname" x={curx + 2} y={height}>
            {(act[1] + "").substring(0, 3)}
          </text>,
        );
        ...
        if (act[3] && act[3].length) {
          for (var m = 0; m < act[3].length; m++) {
            ...
            motifLines.push(
              <line
                className={
                  "annotation annotation__AskingForbidden_" +
                  motif[0] +
                  "_1" +
                  currentClass
                }
        ...
        if (Array.isArray(act[2])) {
          for (var s = 0; s < act[2].length; s++) {
            ...
            divLines.push(
              <text className="scenename" x={curx + 2} y={sceneY + 2}>
