...
class VideoLinks extends Component {
  ...
  isIteration(thing) {
    if (
      typeTest(
        "https://meld.linkedmusic.org/companion/vocab/MotifIteration",
        thing,
      )
    )
      return true;
    if (this.props.iterations.find((x) => x["@id"] === thing["@id"]))
      return true;
    return false;
  }
  isIterationSegment(thing) {
    ...
    if (
      typeTest(
        "https://meld.linkedmusic.org/companion/vocab/MotifIterationSegment",
        thing,
      )
    )
      return true;
    if (
      this.props.iterations.find((x) =>
        x.iterationSegments.find((y) => y["@id"] === thing["@id"]),
      )
    )
      return true;
    return false;
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      for (var j = 0; j < targets.length; j++) {
        if (this.isIteration(targets[j])) {
          ...
          var iterationNo = this.props.iterations.findIndex(
            (x) => x["@id"] == targetID,
          );
          var iteration = this.props.iterations[iterationNo];
          var audio = iteration.embodimentLists.MP3 ? (
            this.videoPlayer(iteration.embodimentLists.MP3[0])
          ) : (
            <div />
          );
          linkMedia[iterationNo] = (
            <div className="motifBlock" key={"vl-" + targetID}>
              ...
              >
                {iteration["http://www.w3.org/2000/01/rdf-schema#label"]}
              </div>
              {audio}
            </div>
          );
          ...
          var targetID = targets[j]["@id"];
          var iterationNo = this.props.iterations.findIndex((x) =>
            x.iterationSegments.find((y) => y["@id"] == targetID),
          );
          var iteration = this.props.iterations[iterationNo];
          var segment = iteration.iterationSegments.find(
            (x) => x["@id"] == targetID,
          );
          var audio = iteration.embodimentLists.MP3 ? (
            this.videoPlayer(iteration.embodimentLists.MP3[0])
          ...
          var segmentName =
            segment["http://purl.org/vocab/frbr/core#realizationOf"][
          ...
          linkMedia[iterationNo] = (
            <div className="motifBlock" key={"vl-" + targetID}>
              <div className="motifName">
                <span
                  className="motif"
                  onClick={this.props.inspectMotive.bind(
                    null,
                    iteration["@id"],
                  )}
                >
          ...
          typeTest(
            "https://meld.linkedmusic.org/companion/vocab/TimeMachine",
            targets[j],
          )
        ) {
          var TM = [
          ...
          typeTest(
            "https://meld.linkedmusic.org/companion/vocab/Compare",
            targets[j],
          )
        ) {
          var LID =
            targets[j][
              "https://meld.linkedmusic.org/companion/vocab/LeftMotif"
            ]["@id"];
          var RID =
            targets[j][
              "https://meld.linkedmusic.org/companion/vocab/RightMotif"
            ]["@id"];
          var iterationL = this.props.iterations.find((x) => x["@id"] == LID);
          var iterationR = this.props.iterations.find((x) => x["@id"] == RID);
          compares.push(
            <div className="motifBlock" key={"c-" + LID + "---" + RID}>
              <div
                className="motifName motifNames"
                onClick={this.props.compare.bind(null, LID, RID)}
              >
                Compare {iterationL[prefix.rdfs + "label"]} &{" "}
                {iterationR[prefix.rdfs + "label"]}
              </div>
            </div>,
          );
        }
  ...
  render() {
    ...
    if (
      this.props.timesync &&
      "mediaResources" in this.props.timesync &&
      ...
      if (syncs) {
        ...
        for (var i = 0; i < times.length; i++) {
          if (
            cT > times[i] &&
            cT < (syncs[times[i]].end ? syncs[times[i]].end : times[i] + window)
          ) {
    ...
    return (
      <div className="videoLinks linksTab">
        ...
        {linkDivs.length == 1 ? linkDivs[0] : linkDivs}
      </div>
    );
  }
}
