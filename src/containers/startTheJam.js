		...
		if(this.props.sessionControl.newSessionUri) { 
			...
			return(
				<div className="sessionControls">
					<div>Score URI: <input type="text" name="scoreUri" value={this.state.scoreUri} onChange={this.handleScoreChange.bind(this)}/></div> 
					<div>Performer URI: <input type="text" name="performerUri" value={this.state.performerUri} onChange={this.handlePerformerChange.bind(this)}/></div>
					<div>Session slug: <input type="text" name="slug" value={this.state.slug} onChange={this.handleSlugChange.bind(this)}/> (optional)</div>
					<button onClick={this.startClimb} disabled={!(this.state.scoreUri && this.state.performerUri)}>Start the climb!</button>
				</div>
			)
		}
	}
