...
export const REGISTER_PUBLISHED_PERFORMANCE_SCORE =
...
export const CADENCE =
  "https://meld.linkedmusic.org/companion/vocab/cadentialGoal";
export const DEGREE = "https://meld.linkedmusic.org/companion/vocab/hasDegree";
export const CHORD_TYPE =
  "https://meld.linkedmusic.org/companion/vocab/chordType";
...
export const HAS_PERFORMANCE_MEDIUM =
  "http://rdaregistry.info/Elements/e/p20215";
export const HAS_PIANO =
  "http://id.loc.gov/authorities/performanceMediums/2013015550";
...
export const IGNORE_TRAVERSAL_OBJECTIVE_CHECK_ON_EMPTY_GRAPH =
  "IGNORE_TRAVERSAL_OBJECTIVE_CHECK_ON_EMPTY_GRAPH";
  ...
  return (dispatch, getState) => {
    ...
    promise
      .then((response) => {
        ...
        if (
          docUri.endsWith(".json") ||
          docUri.endsWith(".jsonld") ||
          docUri.endsWith(".json-ld") ||
          response.headers
            .get("Content-Type")
          ...
          response.headers
            .get("Content-Type")
            .startsWith("application/rdf+xml") ||
          response.headers
            .get("Content-Type")
            .startsWith("application/x-turtle") ||
          ...
          console.log(
            "Can't handle this document: (We currently only support nq and JSON-LD)",
            docUri,
            response,
          );
          ...
          dispatch({ type: TRAVERSAL_FAILED });
          console.log(
            "Don't know how to treat this document: ",
            docUri,
            response,
          );
        }
...
function traverseRDF(dispatch, docUri, params, dataPromise) {
  console.log(
    "in traverseRDF for doc ",
    docUri,
    "with exclude list ",
    params["ignoreObjectUri"],
  );
    ...
    jsonld
      ...
      .then((expanded) => {
        ...
        jsonld.flatten(expanded).then((flattened) => {
          ...
          Object.entries(idLookup).forEach(
            ([subjectUri, subjectDescription]) => {
...
function passesTraversalConstraints(obj, params, predicate) {
  ...
  if (params["extendObjectUri"].length) {
    ...
    if (
      !params["extendObjectUri"].includes(resourceUri) ||
  ...
  if (predicate && params["followPropertyPrefix"].length) {
    if (
      !params["followPropertyPrefix"].some((pref) => predicate.startsWith(pref))
    ) {
...
export function fetchSessionGraph(uri, etag = "") {
  console.warn(
    "DEPRECATION WARNING: The function fetchSessionGraph is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
    ...
    promise.then((response) => {
      ...
      if (!etag) {
        ...
        if ("mo:performance_of" in session) {
          dispatch(
            fetchConceptualScore(
              session["@id"],
              session["mo:performance_of"]["@id"],
            ),
          );
        } else {
...
function processComponentAnnotation(annotation, conceptualScore = "") {
  console.warn(
    "DEPRECATION WARNING: The function processComponentAnnotation is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
  if (
    "meld:state" in annotation &&
    annotation["meld:state"]["@id"] === "meld:processed"
  ) {
...
export function fetchComponentTarget(uri, conceptualScore = "") {
  console.warn(
    "DEPRECATION WARNING: The function fetchComponentTarget is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
  ...
  return (dispatch, getState) => {
    ...
    promise.then((data) => {
      ...
      if (
        ...
        jsonld
          .fromRDF(data.data, { format: "application/n-quads" })
          .then((doc) => {
...
function processComponentTarget(data, uri, conceptualScore) {
  console.warn(
    "DEPRECATION WARNING: The function processComponentTarget is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the checkTraversalObjectives function instead.",
  );
      ...
      .then((framed) => {
        jsonld
          ...
          .then((compacted) => {
            ...
            if (
              typecheck["@type"].includes(SEGMENT) ||
              typecheck["@type"].includes(MUZICODE)
            ) {
...
export function fetchTargetExpression(compacted) {
  console.warn(
    "DEPRECATION WARNING: The function fetchTargetExpression is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
  ...
  return (dispatch) => {
    ...
    if (target["@type"].includes(EXPRESSION)) {
      ...
      if (CADENCE in target) {
        ...
        if (DEGREE in cadenceData)
          expressionObj.cadence.degree = cadenceData[DEGREE]["@id"];
        if (CHORD_TYPE in cadenceData)
          expressionObj.cadence.chordType = cadenceData[CHORD_TYPE]["@id"];
      }
      ...
      console.log(
        "fetchTargetExpression attempted on a non-Expression! ",
        target,
      );
    }
  };
...
export function fetchWork(target, parts, work, expressionObj) {
  console.warn(
    "DEPRECATION WARNING: The function fetchWork is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
              ...
              jsonld.compact(framed, context, (err, compacted) => {
                if (err) {
                  ...
                  if (HAS_STRUCTURE in work) {
                    dispatch(
                      fetchStructure(target, parts, work[HAS_STRUCTURE]["@id"]),
                    );
                    ...
                    jsonld.frame(
                      ...
                      (err, framed) => {
                        if (err) {
                          console.log(
                            "FRAMING ERROR when fetching parent work",
                            err,
                          );
                        } else {
                          ...
                          if (
                            attachedScore &&
                            "@type" in attachedScore &&
                            attachedScore["@type"] === SCORE
                          ) {
                                ...
                                if (HAS_PERFORMANCE_MEDIUM in pubScore) {
                                  ...
                                  dispatch({
                                    ...
                                    payload: {
                                      ...
                                      performanceMedium:
                                        pubScore[HAS_PERFORMANCE_MEDIUM],
                                    },
                                  });
                                  if (
                                    pubScore[HAS_PERFORMANCE_MEDIUM]["@id"] ==
                                    HAS_PIANO
                                  ) {
                                    dispatch(fetchScore(pubScore["@id"]));
                                  } else {
                                    dispatch(
                                      fetchRibbonContent(pubScore["@id"]),
                                    );
                                  }
                                } else {
                            ...
                            if (HAS_STRUCTURE in attachedScore) {
                              dispatch(
                                fetchStructure(
                                  target,
                                  parts,
                                  attachedScore[HAS_STRUCTURE]["@id"],
                                ),
                              );
                            } else {
                            ...
                            dispatch(
                              fetchWork(target, parts, work[PART_OF]["@id"]),
                            );
                          }
                        }
                      },
...
export function fetchStructure(target, parts, segline) {
  console.warn(
    "DEPRECATION WARNING: The function fetchStructure is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
  return (dispatch, getState) => {
    const fetchFn = getCurrentFetch(getState());
...
export function fetchConceptualScore(session, uri) {
  console.warn(
    "DEPRECATION WARNING: The function fetchConceptualScore is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead.",
  );
  return (dispatch, getState) => {
    const fetchFn = getCurrentFetch(getState());
    const promise = fetchFn(uri, {
...
export function postAnnotation(
  ...
  return (dispatch, getState) => {
    ...
    if (retries) {
      ...
      console.log(
        "FAILED TO POST ANNOTATION (MAX RETRIES EXCEEDED): ",
        session,
        etag,
        json,
      );
...
export function markAnnotationProcessed(
  ...
  if (retries) {
    ...
    console.log(
      "FAILED TO PATCH ANNOTATION (MAX RETRIES EXCEEDED): ",
      session,
      etag,
      annotation,
    );
...
export function patchAndProcessAnnotation(
  action,
  session,
  etag,
  annotation,
  success = { type: ANNOTATION_PATCHED },
  retries = MAX_RETRIES,
) {
  if (retries) {
    // console.log("PATCHING: ", session, etag, annotation);
