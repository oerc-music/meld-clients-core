...
export function traverse(docUri, params) {
  ...
  return dispatch => {
    ...
    promise.then(response => {
      ...
      console.debug(response.headers.get("Content-Type")); // attempt to decide content type (either explicitly provided or by file suffix)
      ...
      if (docUri.endsWith(".json") || docUri.endsWith(".jsonld") || docUri.endsWith(".json-ld") || response.headers.get("Content-Type").startsWith("application/ld+json") || response.headers.get("Content-Type").startsWith("application/json")) {
        ...
        console.debug("Can't handle this document: (We currently only support nq and JSON-LD)", docUri, response); // dispatch(traverseRDF(dispatch, docUri, params, response.text()));
...
function traverseRDF(dispatch, docUri, params, dataPromise) {
  console.debug("in traverseRDF for doc ", docUri, "with exclude list ", params["ignoreObjectUri"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes
...
function traverseJSONLD(dispatch, docUri, params, dataPromise) {
  console.debug("in traverseJSONLD for doc ", docUri, "with exclude list ", params["ignoreObjectUri"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes

  dataPromise.then(data => {
    console.debug("attempting to expand: ", data);
    jsonld.expand(data).then(expanded => {
      console.debug("Got expanded json: ", expanded); // flatten the expanded JSON-LD object so that each described entity has an ID at the top-level of the tree
...
export function fetchSessionGraph(uri, etag = "") {
  console.warn("DEPRECATION WARNING: The function fetchSessionGraph is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead."); // console.debug("FETCH_SESSION_GRAPH ACTION ON URI: ", uri, " with etag: ", etag);
...
function processComponentAnnotation(annotation, conceptualScore = "") {
  ...
  annotation = ensureArray(annotation, "oa:hasTarget"); // console.debug("Processing component annotation: ", annotation, conceptualScore)
...
export function fetchComponentTarget(uri, conceptualScore = "") {
  console.warn("DEPRECATION WARNING: The function fetchComponentTarget is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead."); // console.debug("FETCH_COMPONENT_TARGET ACTION ON URI: ", uri);
...
export function fetchTargetExpression(compacted) {
  ...
  return dispatch => {
    ...
    if (target["@type"].includes(EXPRESSION)) {
      ...
      let parts = []; // console.debug("part check: ", target)
...
export function fetchWork(target, parts, work, expressionObj) {
  console.warn("DEPRECATION WARNING: The function fetchWork is considered deprecated as of meld-clients-core v2.0.0 and will be subject to removal in future versions. Please upgrade your application to use the registerTraversal and traverse functions instead."); // console.debug("STARTING FETCHWORK WITH ", work, parts, expressionObj);
            ...
            if (err) {
              ...
              jsonld.compact(framed, context, (err, compacted) => {
                if (err) {
                        ...
                        if (attachedScore && "@type" in attachedScore && attachedScore["@type"] === SCORE) {
                          ...
                          if (PUBLISHED_AS in attachedScore) {
                            ...
                            attachedScore[PUBLISHED_AS].map(pubScore => {
                              ...
                              } else {// console.debug("Published score without performance medium: ", pubScore["@id"]);
                              }
                            });
                          } else {// console.debug("Unpublished score: ", attachedScore);
                          }

                          if (HAS_STRUCTURE in attachedScore) {
                            dispatch(fetchStructure(target, parts, attachedScore[HAS_STRUCTURE]["@id"]));
                          } else {// console.debug("Score ", attachedScore["@id"], " attached to work ", work["@id"], " has no segment line!!");
                          }
                        } else {
                  ...
                  } else {// console.debug("Found work without segmentLine or partonomy! ", work);
                  }
                }
              });
            }
    ...
    if (retries) {
      ...
      auth.fetch(session, {
        ...
        if (error.response.status == 412) {
          console.debug("Mid-air collision while attempting to POST annotation. Retrying.", session, etag, json); // GET the session resource to figure out new etag
...
export function markAnnotationProcessed(session, etag, annotation, retries = MAX_RETRIES) {
  if (retries) {
    ...
    axios.patch(session, patchJson, {
      ...
      if (error.response.status == 412) {
        console.debug("Mid-air collision while attempting to MARK annotation processed. Retrying.", session, etag, annotation); // GET the session resource to figure out new etag
  ...
  if (retries) {
    ...
    return dispatch => {
      axios.patch(session, patchJson, {
        ...
        dispatch(action); // console.debug("Dispatching success callback: ", success)

        dispatch(success);
      }).catch(function (error) {
        if (error.response.status == 412) {
          console.debug("Mid-air collision while attempting to PATCH annotation. Retrying.", session, etag, annotation); // GET the session resource to figure out new etag
