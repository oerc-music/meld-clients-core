...
export function registerTraversal(docUri, suppliedParams = {}) {
  ...
  const unimplementedParams = ("extendObjectType", "ignoreObjectType", "followPropertyPrefix", "followPropertyUri", "ignorePpropertyPrefix", "ignorePropertyUri");
...
function traverseJSONLD(dispatch, docUri, params, dataPromise) {
  console.log("in traverseJSONLDf or doc ", docUri, "with exclude list ", params["ignoreObjectUri"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes

  dataPromise.then(data => {
    console.log("attempting to expand: ", data);
    jsonld.expand(data, (err, expanded) => {
      console.log("traverseJSOMNLD jsonld.expand callback", err, expanded);
