...
export function registerTraversal(docUri, {
...
export function traverse(docUri, params) {
  ...
  return dispatch => {
    ...
    promise.then(response => {
      ...
      let data = response.data; // attempt to decide content type (either explicitly provided or by file suffix)
      // and proceed with traversal accordingly

      if (docUri.endsWith(".json") || docUri.endsWith(".jsonld") || docUri.endsWith(".json-ld") || response.headers["content-type"].startsWith("application/ld+json") || response.headers["content-type"].startsWith("application/json")) {
        // treat as JSON-LD document
        dispatch(traverseJSONLD(dispatch, docUri, params, response.data));
      } else if (docUri.endsWith(".ttl") || docUri.endsWith(".n3") || docUri.endsWith(".rdf") || docUri.endsWith(".nq") || docUri.endsWith(".nt") || response.headers["content-type"].startsWith("application/rdf+xml") || response.headers["content-type"].startsWith("application/nquads") || response.headers["content-type"].startsWith("application/x-turtle") || response.headers["content-type"].startsWith("text/turtle")) {// treat as RDF document
...
function passesTraversalConstraints(obj, params) {
  ...
  if (params["objectUriWhitelist"].length) {
    ...
    if (!obj["@id"] in params["objectUriWhitelist"] || obj["@id"] in params["objectUriBlacklist"]) {
      return false;
    }
  } else {
    // no URI whitelist
...
export function fetchComponentTarget(uri, conceptualScore = "") {
  ...
  return dispatch => {
    promise.then(data => {
      ...
      if (!"content-type" in data.headers || data.headers["content-type"] !== "application/json" && data.headers["content-type"] !== "application/ld+json") {
    ...
    if (retries) {
      ...
      axios.post(session, json, {
        ...
        if (error.response.status == 412) {
          console.log("ARF Mid-air collision while attempting to POST annotation. Retrying.", session, etag, json); // GET the session resource to figure out new etag
