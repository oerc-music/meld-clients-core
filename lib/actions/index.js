...
function traverseRDF(dispatch, docUri, params, dataPromise) {
  console.log("in traverseRDF for doc ", docUri, "with blacklist ", params["objectUriBlacklist"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes
...
function traverseJSONLD(dispatch, docUri, params, dataPromise) {
  console.log("in traverseJSONLD for doc ", docUri, "with blacklist ", params["objectUriBlacklist"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes

  dataPromise.then(data => {
    // If it's not a string, then it's a JSONLD object. Turn it into string
