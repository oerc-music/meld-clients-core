...
export default class IIIF extends Component {
  constructor(props) {
    ...
    const iiifTileLayer = "iiifTileLayer" in this.props ? this.props.iiifTileLayer : React.createElement(IIIFTileLayer, {
      url: this.props.url
    });
    const map = "map" in this.props ? this.props.map : React.createElement(Map, {
      className: className,
      center: [lat, lng],
      zoom: zoom,
