...
import { prefix as p } from '../library/prefixes.js'
...
class TEI extends Component {
  ...
  componentDidUpdate() {
    ...
    this.props.annotations.map(annotation => {
      // each annotation...
      const frags = annotation[p.oa+"hasTarget"].map(annotationTarget => {
  ...
  handleMELDActions(annotation, fragments) {
    if (p.oa+"hasBody" in annotation) {
      annotation[p.oa+"hasBody"].map(b => {
