...
import { MARKUP_EMPHASIS, handleEmphasis, MARKUP_HIGHLIGHT, handleHighlight, MARKUP_HIGHLIGHT2, handleHighlight2, CUE_IMAGE, handleCueImage } from '../actions/meldActions';
...
class TEI extends Component {
  ...
  render() {
    ...
    return React.createElement("div", null, " Loading TEI... ");
  }
  ...
  returnHTMLizedTEI() {
    return {
      __html: this.props.tei.TEI[this.props.uri].innerHTML
    };
  }
    ...
    this.props.annotations.map(annotation => {
      ...
      const frags = annotation["oa:hasTarget"].map(annotationTarget => {
        ...
        if (annotationTarget["@id"] in this.props.tei.componentTargets) {
          ...
          const myFrags = this.props.tei.componentTargets[annotationTarget["@id"]].filter(frag => {
            ;
