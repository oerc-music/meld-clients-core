...
class OrchestralRibbon extends Component {
  ...
  render() {
    if (Object.keys(this.props.score).length) {
      if (true && this.props.score["MEI"] && this.props.score["MEI"][this.props.uri]) {
        var orch = new Orchestration(this.props.score["MEI"][this.props.uri], this.props.additionalInstruments);
        var startx = 120;
        var starty = 20;
        ...
        for (var i = 0; i < stop; i++) {
          ...
          captions.push(cap);
          boxes.push(drawRibbons(orch.instruments[i].onOffArray(), ypos, rowHeight, (width - 120) / orch.measureCount, ' ' + inst.section + " " + inst.name + " nnn" + i, undefined, undefined, i, 120));
        }
